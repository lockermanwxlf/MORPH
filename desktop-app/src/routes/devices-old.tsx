...
function BluetoothDeviceActions({
	onOpenHotspot,
	onOpenBluetooth,
}: {
	onOpenHotspot: () => void | Promise<void>;
	onOpenBluetooth: () => void | Promise<void>;
}) {
	return (
		<div className="mt-0 max-h-0 overflow-hidden opacity-0 transition-all duration-200 group-hover:mt-4 group-hover:max-h-48 group-hover:opacity-100">
			<p className="test-xs test-[var(--ink-1)]">
				Communication between devices requires wifi protocols. This device is
				nearby, but is either not on the same network as you, or your wifi
				blocks peer-to-peer connections or multicast DNS.
			</p>
...
function WifiDeviceAction({
	...
	deviceId: string;
	tisConnecting: boolean;
	tconnectedDeviceId?: string | null;
	tconnectToDevice: (id: string) => void | Promise<void>;
}) {
	return (
		<div className="mt-0 max-h-0 overflow-hidden opacity-0 transition-all duration-200 group-hover:mt-4 group-hover:max-h-24 group-hover:opacity-100">
			<button
				type="button"
				disabled={tisConnecting}
				onClick={() => {
					void tconnectToDevice(deviceId);
				}}
...
function BluetoothDeviceListItem({ device }: { device: BluetoothDevice }) {
	const hostSID = useHostSSID();
	return (
		<li className="group rounded-xl border border-[var(--line)] bg-black/20 p-4 transition-colors hover:bg-black/30">
			<div className="flex items-start justify-between gap-3">
				<div className="min-w-0">
					<p className="truncate text-base font-semibold">{device.title}</p>
...
function DeviceListItem({
	...
	connectToDevice: (id: string) => void | Promise<void>;
	onOpenHotspot: () => void | Promise<void>;
	onOpenBluetooth: () => void | Promise<void>;
}) {
	return (
		<li className="group rounded-xl border border-[var(--line)] bg-black/20 p-4 transition-colors hover:bg-black/30">
			...
			{device.source === "bluetooth" ? (
				...
				<WifiDeviceAction
					...
					tisConnecting={isConnecting}
					tconnectedDeviceId={connectedDeviceId}
					tconnectToDevice={connectToDevice}
				/>
			)}
		</li>
	);
					...
					<ul className="grid flex-1 auto-rows-min content-start gap-4 overflow-auto sm:grid-cols-2 lg:grid-cols-3">
						{unifiedDevices.map((device) => (
							<DeviceListItem
								...
								onOpenHotspot={handleOpenHotspotSettings}
								onOpenBluetooth={handleOpenBluetoothSettings}
							/>
						))}
					</ul>
				)}
